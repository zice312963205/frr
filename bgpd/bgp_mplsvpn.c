...
void ensure_vrf_tovpn_sid_static(struct bgp *bgp_vpn, struct bgp *bgp_vrf, afi_t afi)
{
	int debug = BGP_DEBUG(vpn, VPN_LEAK_FROM_VRF);
	char buf[256];
	...
	tovpn_sid_locator = locator_lookup_by_name(bgp_vpn->srv6_locators_hash, bgp_vrf->srv6_locator_name);
    if (!tovpn_sid_locator)
    {
		zlog_err("%s: locator %s do not exist.", __func__, bgp_vrf->srv6_locator_name);
        return;
    }
    
    sid = sid_lookup_by_vrf(tovpn_sid_locator, bgp_vrf->name, afi);
    if (!sid)
    {
        if (bgp_vrf->tovpn_sid)
            XFREE(MTYPE_BGP_SRV6_SID, bgp_vrf->tovpn_sid);
        bgp_vrf->tovpn_sid_locator_ex = NULL;
		zlog_err("%s: opcode do not exist for vrf %s under locator %s", 
			__func__, bgp_vrf->name_pretty,bgp_vrf->srv6_locator_name);
        return;
    }
	
	tovpn_sid = XCALLOC(MTYPE_BGP_SRV6_SID, sizeof(struct in6_addr));
    combine_sid(tovpn_sid_locator, &sid->ipv6Addr.prefix, tovpn_sid);
	tovpn_sid_transpose_label = alloc_new_sid_ex(bgp_vpn, sid, tovpn_sid_locator);
...
void ensure_vrf_tovpn_sid(struct bgp *bgp_vpn, struct bgp *bgp_vrf, afi_t afi)
{
	...
	if (CHECK_FLAG(bgp_vrf->vrf_flags, BGP_VRF_TOVPN_SID_STATIC))
		return ensure_vrf_tovpn_sid_static(bgp_vpn, bgp_vrf, afi);
}
...
void delete_vrf_tovpn_sid_per_vrf(struct bgp *bgp_vpn, struct bgp *bgp_vrf)
{
	...
	/* skip when VPN is configured on vrf-instance */
	if (tovpn_sid_index != 0 || tovpn_sid_auto || tovpn_sid_static)
		return;
	...
	if (bgp_vrf->tovpn_sid && (tovpn_sid_index==0 || !tovpn_sid_auto)) {
		ctx.vrf_id = bgp_vrf->vrf_id;
		ctx.behavior = ZEBRA_SEG6_LOCAL_ACTION_END_DT46;
		bgp_zebra_release_srv6_sid(&ctx);
	...
	if (bgp_vrf->tovpn_sid && !tovpn_sid_static){
		sid_unregister(bgp_vpn, bgp_vrf->tovpn_sid);
		XFREE(MTYPE_BGP_SRV6_SID, bgp_vrf->tovpn_sid);
	}
			      ...
			      struct bgp_path_info *path_vrf) /* route */
{
	...
	if (from_bgp->vpn_policy[afi].tovpn_sid_locator) {
	...
	} else if (from_bgp->tovpn_sid_locator && !CHECK_FLAG(bgp_vrf->vrf_flags, BGP_VRF_TOVPN_SID_STATIC)) {
		...
		memcpy(&static_attr.srv6_l3vpn->sid,
		       ...
		       sizeof(struct in6_addr));
	} else if(from_bgp->tovpn_sid_locator && from_bgp->tovpn_sid && CHECK_FLAG(bgp_vrf->vrf_flags, BGP_VRF_TOVPN_SID_STATIC)){
		struct srv6_locator * locator = from_bgp->tovpn_sid_locator;
		encode_label(from_bgp->tovpn_sid_transpose_label, &label);
		static_attr.srv6_l3vpn =
			XCALLOC(MTYPE_BGP_SRV6_L3VPN,
...
void vpn_leak_postchange_checksid(void)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bm->bgp, next, bgp)) {
		...
		if (sid4) {
			...
			if (sid_same(bgp->tovpn_sid, &sid_buf))
				sid4_skip = true;
		}
		if (sid6) {
			memset(&sid_buf, 0x0, sizeof(struct in6_addr));
